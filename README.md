Работу выполнил: Стафеев Иван Алексееич (https://t.me/st_affeev), группа K3321
...
Написанные тесты по большей части соответствуют принципу AAA, однако местами имеются дополнительные assert'ы в части Arrange, необходимые для дополнительной проверки связи GUI и БД. Плюс из-за моков некоторые тесты выглядят достаточно нагруженно, что, однако, можно исправить небольшим рефактором кода.
...
3) Repeatable: да, в тестах не используются случайные значения. для каждого теста создается новая in-memory БД, а для нестаильных велиичин, таких как текущая дата, созданы моки, что обеспечивает повторяемость.
