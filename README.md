Работу выполнил: Стафеев Иван Алексеевич (https://t.me/st_affeev), группа K3321
...
![how programm works](img/prog_work.gif)

### Ключевые точки интеграции в проекте

В проекте интеграционные точки формируются на пересечении GUI, бизнес-логики и базы данных.

##### 1. GUI ↔ База данных

- добавление покупки: GUI получает данные от пользователя, проверяет их, вызывает CRUD-функции для записи в БД, обновляет список покупок, баланс и фильтры категорий.  

- изменение покупки: GUI передает новые данные, обновляется запись в БД, при необходимости создается новая категория, GUI отображает изменения.  

- удаление покупки или категории: GUI начинает удаление, CRUD-функции обновляют БД, GUI обновляет таблицу, фильтры, баланс и сумму затрат.

##### 2. CRUD-операции ↔ ORM-модели

- все операции с базой используют ORM-модели `Category` и `Product`.  
- исключения при нарушении правил (дублирование имени, пустое имя) перехватываются на уровне GUI.  
- сортировка и фильтры используют данные из БД для корректного отображения в GUI.

##### 3. Ключевые пользовательские сценарии

- пополнение баланса: обновляется баланс, GUI показывает новые данные. 
- добавление покупки с новой категорией: создается запись покупки и категория, GUI обновляет список и фильтры, баланс уменьшается.
- изменение покупки: обновляются данные в БД и GUI, при необходимости добавляется новая категория.  
- удаление покупки или категории: удаляются записи в БД, GUI корректно обновляет список и фильтры.  
- фильтрация и сортировка: GUI отображает данные из БД по выбранным критериям.

### Написание тестов

#### 0. Подготовка к тестированию (`test_setup.py`)

Во-первых, определимся с библиотекой для тестирования. Естественным решением сначала казался `pytest`, однако тестирование GUI на PyQt на моем устройстве при помощи библиотеки `pytest-qt` оказалось не выполнить в силу ошибок с DLL-файлами. После n-го количество попыток исправить ошибку было принято решение написать тесты на `unittest`, во-первых, чтобы не возиться с проблемой, которая не является острой на данный момент, а во-вторых, чтобы попробовать и другой инструмент, кроме `pytest`, который использовался в лабораторной работе №1.
...
В этом классе происходит инициализация in-memory базы данных, а также мок текущей даты в PyQt и мок диалоговых окон PyQt, чтобы они не перехватывали поток событий. В методах `tearDownClass` и `tearDown` происходит отключение моков и закрытие сессии БД. От этого класса будут наследоваться все дальнейшие классы, содержащие тесты.

Общая логика работы отдельного теста заключается в следующем: создание приложение PyQt → инициализация in-memory БД и моки → проведение теста → отключение моков и завершение сессии БД → закрытие приложения PyQt.
...
def test_add_category_long_name(self):
    """Добавление категории с слишком длинным именем"""
    with self.assertRaises(Exception):
...
Для тестирования взаимодействия GUI ↔ База данных в разрезе изменения покупок был написан класс `TestGuiEditing(TestSetupMixin)`. Внутри него два теста: 1) изменение данных о покупке без изменения категории; 2) изменение данных о покупке с созданием новой категории.
...
Для тестирования взаимодействия GUI ↔ База данных в разрезе удаления покупок был написан класс `TestGuiDeleting(TestSetupMixin)`. Внутри него два теста: 1) удаление категории покупки; 2) удаление покупки. 
...
Первый метод нужен для мока текущего datetime. В силу иммутабельности `datetine.now()` было решено сделать локальный мок в одном (этом) классе, вызываемый при каждом тесте, а не общий мок в сетапе, как было с датой из PyQt. Второй метод возвращает данные из списка покупок для assert'ов.
...
Здесь приведены тесты сортировки по убыванию/возрастанию даты/стоимости покупки. Везде просто проверяется соответствие отображаемых покупок в списке с логикой того, как они реально должны быть отображены с использованием конкретного типа сортировки.
...
Для автоматизации был написан [action](.github/workflows/testing.yml), который при пуше в ветку вызывает запуск тестов через `green -vvv --run-coverage` (фреймворк `green` используется для компактного и наглядного вывода результатов тестов).

Тесты писались последовательно (в том порядке, в котором указаны в отчете), и в них уже присутствовали тесты, которые не прошли, но исправление соответствующих ошибок в коде было оставлено на потом.
...
Валидация имени в ORM-модели покупки реализована так:
...
В отличие от модели категории покупки, здесь было пропущено условие на ненулевое имя, поэтому в тестах добавление/изменение покупки с пустым именем не выдавало ошибку. Валидацию необходимо изменить на...
...
В результате исправления теперь все тесты проходят:

![запуски тестов](./img/run3.png)

![запуски тестов](./img/run4.png)

### Анализ написанных тестов

После исправления ошибок в коде все тесты проходят без падений. С помощью `green --run-coverage` было посчитано покрытие тестами программного кода:

![code coverage](./img/run4.png)

Для файлов, относящихся в работе с базой данных (ORM-модели, CRUD-операции) показатель покрытия достаточно высокий (>= 89%), для файлов, относящихся к реализации GUI, этот показатель значительно ниже (вплоть до 20% у `gui/form_add-_purchase.py`). Однако общий показатель составляет `71%`, что можно назвать хорошим результатом для интеграционного тестирования (как минимум, у нас нет необходимости проверять все функции, относящиеся к GUI, поскольку далеко не все из них связывают разные модули и посему относятся к unit-тестированию, которое нас не интересует в этой работе). 

Написанные тесты покрывают все предусмотренные техническим заданием варианты использования программы. Во время написания тестов были найдены реальные недочеты и ошибки в логике кода, которые были исправлены.

Написанные тесты по большей части соответствуют принципу AAA, однако местами имеются дополнительные assert'ы в части Arrange, необходимые для дополнительной проверки связи GUI и БД. Плюс из-за моков некоторые тесты выглядят достаточно нагружено, что, однако, можно исправить небольшим рефактором кода.

Принципам FIRST они тоже соответствуют (почти):
1) Fast: 29 тестов выполняется за ~2.1 секунду - достаточно быстро
2) Isolated: да, отдельные тесты и блоки тестов не зависят друг от друга, так как в каждом тесте создается новое подключение к БД (и затем оно чистится) и новое приложение PyQt App.
3) Repeatable: да, в тестах не используются случайные значения. для каждого теста создается новая in-memory БД, а для нестабильных величин, таких как текущая дата, созданы моки, что обеспечивает повторяемость.
4) Self-validating: да, благодаря assert тесты сами сообщают, успешно они пройдены или нет
5) Timely: нет, так как тесты писались уже после написания кода программы, однако такое было задание у лабораторной работы
